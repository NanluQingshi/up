...
这一章节是 RAG 模块的最终章，这里我们会将 RAG 再往更好的完成度推进，加入 llm 改写和 chat history，成为更成熟的 llm 应用。 
## llm 改写提问
---
...
这里，我们==**将 model 的 temperature 定义的较低，越低 llm 会越忠于事实，减少自己的自由发挥。** ==
然后，让我们简单测试一下效果：
...
我们首先准备一个独立的脚本去对给定的小说文本进行切割，并保存在本地的数据库文件中，这段代码的细节前面都讲解过了，所以我们直接贴代码：
...
然后，我们构建 ==根据重写后的独立问题去读取数据库中相关文档== 的 chain:
...
这里，我们使用 `MessagesPlaceholder` 在 message 中给 history 预留位置，之后会这里会被 Message 数组填充。  
