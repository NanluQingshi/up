前面的课程讲了一些基础的内容，本节课来讲解一下 Node.js 的启动过程 ，这是我们深入理解 Node.js 工作原理非常重要的一步，了解了启动过程，我们才能更好地了解后面的核心模块是如何在启动的进程中工作的。本节课先从宏观介绍 Node.js 的启动过程，然后再详细分析启动过程中各个核心部分的实现，从中我们可以看到启动过程中每一个核心的步骤以及它的作用。

  


开始讲解前，我们先来看一下 Node.js 启动时的核心流程，有了大致印象后，会更容易理解后面的内容，同时也可以通过这个流程图不断地加深对启动过程的理解。

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/bfc449cb3a4c4fe5a3af71e602f1cb17~tplv-k3u1fbpfcp-zoom-1.image)
...
![image.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/95fb0fd34d6a49888743847c753d6283~tplv-k3u1fbpfcp-watermark.image?)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/7c232d43fe954d55a0e4127bb4f1fbd1~tplv-k3u1fbpfcp-watermark.image?)
...
2.  immediate_check_handle 是实现 Node.js 中 setImmediate 的数据结构，对应 Libuv 的check 阶段。

<!---->

3.  immediate_idle_handle 也是实现 Node.js 中 setImmediate 的数据结构，对应 Libuv 的 idle 阶段。

<!---->

4.  task_queues_async_ 用于子线程和主线程通信。
...
1.  C++ 模块用于暴露底层的能力到 JS 层。

<!---->

2.  Platform 用于管理多个子线程辅助 Node.js 的工作，比如 GC。

<!---->

3.  Environment 用于管理 Node.js 中的公共数据结构和逻辑。

<!---->

4.  Loader 用于初始化模块加载器，为后续执行代码做准备。

<!---->

5.  执行用户的 JS 注册任务到事件循环。

<!---->

6.  最后一个步骤是启动事件循环，这样 Node.js 就启动起来了。
