在《Node.js 基础架构》这节课我们讲过，Node.js 是由 V8、Libuv 和 C++ 胶水层组成的。那么 C++ 胶水层是如何把 Libuv 的功能引入 JS 的呢？这节课我们就来好好聊一聊。

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f7b86cd00b30426b9c183e500e83badc~tplv-k3u1fbpfcp-zoom-1.image)

# V8 基础知识

我们知道，JS 的拓展能力是由 V8 提供的。也就是说，在 Libuv 实现了网络、文件、进程等功能后，还需要借助 V8 提供的拓展能力引入 JS 中，才能在 JS 里使用相关的功能。因此，我们需要先了解 V8 的一些基础数据结构和提供的拓展机制。
...
1.  通过 FunctionTemplate::New(isolate) 创建一个函数模板，函数模板是定义了当通过这个函数模板创建一个函数时函数的内容，比如函数名，函数的原型对象里有什么属性等，对应 JS 的函数。

<!---->

2.  通过 SetClassName 定义了当通过这个函数模板创建一个函数时，这个函数的名字。

<!---->

3.  通过设置 PrototypeTemplate 的属性定义了当通过这个函数模板创建一个函数时，这个函数的原型对象，就是 JS 里的 function.prototype。

<!---->

4.  通过设置 InstanceTemplate 的属性定义了当通过这个函数模板创建一个函数并且通过 new 执行这个函数时，生成的对象里有哪些内容。
...
可以看到，通过 Parent 创建出来的对象具备了 PrototypeTemplate 和 InstanceTemplate 里的属性，结构图如下所示：

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/034c3e28dbec4cb0b5e0ffe5cd6782d5~tplv-k3u1fbpfcp-zoom-1.image)
...
可以看到，child 继承了 parent PrototypeTemplate 中的内容，但是没有继承 InstanceTemplate 的内容。结构图如下：

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/09fd8d25e14c459e988852cf0815932f~tplv-k3u1fbpfcp-zoom-1.image)
...
1.  创建一个函数模块，和之前不一样的是，这里会设置一个回调函数是 New，并执行 InstanceTemplate()->SetInternalFieldCount(1)，因为我们需要在 JS 对象中关联一个 C++ 对象，所以需要设置为 1，类似预留一个 slot。

<!---->

2.  当 JS 执行 new FuncWithCallback 时，C++ 层首先会创建一个 JS 对象，然后调用 New 函数，在 New 函数中可以通过 args.This() 拿到这个 JS 对象。

<!---->

3.  接着创建一个 Dummy 对象，把这个 JS 对象传入 Dummy 的构造函数中。Dummy 构造函数在 jsObject 字段中保存了 JS 对象，然后在 JS 对象中保存了 Dummy 对象，最后往 JS 对象新增了一个hello 属性，结构图如下：

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/56540ec6802a4b20806042a0689b8b51~tplv-k3u1fbpfcp-zoom-1.image)

4.  JS 层 new 执行完毕拿到了一个 JS 对象 instance，接着执行 instace.method() 时就会调用 C++ 的 Method。Method 函数中可以通过 args.Holder() 或 args.This()（通常是一样的）获得函数调用的上下文，类似 JS 函数调用时的 this。

<!---->

5.  接着通过 GetAlignedPointerFromInternalField(0) 获得之前关联的 C++ 对象。从而取得相应的内容。

结构图如下。

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3a5e492c2e864a7d993227537b841440~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e44a715306fd4e7e9aeb58d0b98ee624~tplv-k3u1fbpfcp-zoom-1.image)
...
```c++
inline v8::MaybeLocal<v8::Value> AsyncWrap::MakeCallback(const v8::Local<v8::Name> symbol,  
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/00b382eba34b4aac8d3faa60862829d9~tplv-k3u1fbpfcp-zoom-1.image)
...
MakeLibuvRequestCallback::For 会遍历传入的参数，如果是非函数参数，则透传给 Libuv，如果是函数参数时会执行第二个 For 函数，第二个 For 函数封装了原始的回调，然后把一个 Wrapper 函数传入 Libuv，等到 Libuv 回调时，再执行真正的回调，通过这种劫持的方式，C++ 层可以做一些额外的事情。执行 Dispatch 后的结构图如下所示。

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/17e317f669064e3189758c93e84373b4~tplv-k3u1fbpfcp-zoom-1.image)
...
当发起一个 TCP 连接时，使用方式如下，后面的课程会详细介绍。
...
前面讲过，JS 本身是没有网络、文件和进程这些功能的，在 Node.js 里，这些功能需要通过底层的 C、C++ 实现，最终通过 JS API 提供给用户使用。那么当我们调用一个 JS API 时，底层发生了什么呢？我们以 TCP 模块为例，下面的代码摘自 Node.js 的 net 模块。
...
前面讲过，SetAlignedPointerInInternalField 函数做的事情就是把一个值（TCPWrap对象）保存到 JS 对象 object 里。如下图所示。

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/22bb7e0733df440c8f06202aa324638e~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/9ed9b548b614453b9daae6774d9a4a3f~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/041e1c250c7e4b4782c9ebb9d9e8cc91~tplv-k3u1fbpfcp-zoom-1.image)
...
Libuv 中保存了请求上下文，比如回调，并把 req 和 handle 做了关联，在执行回调时会使用，如下图所示。

![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/64a3646c100e4044844a8b0b4accab57~tplv-k3u1fbpfcp-zoom-1.image)
...
MakeCallback 中通过 ConnectWrap 的 object() 函数获取关联的 JS 对象，也就是 JS 创建的 TCPConnectWrap。
...
MaybeLocal<Value> AsyncWrap::MakeCallback(const Local<Function> cb,
  ...
  InternalMakeCallback(env(), GetResource(), object(), cb, argc, argv, context);
}
...
1.  Node.js 通过 V8 提供的函数模版和对象模版来拓展 JS 的功能。

<!---->

2.  C++ 层的核心数据结构大多数是 C++ 类的基类，它们封装了很多通用的逻辑。BaseObject 用于 JS 和 C++ 对象的管理，AsyncWrap 用于异步回调 JS，HandleWrap 和 ReqWrap 是对 Libuv handle 和 request 的封装，这是我们必须理解的核心数据结构，后面的课程中会大量引用。

<!---->

3.  我们沿着 JS 到 C++ 再到 Libuv，然后从 Libuv 到 C++，再到 JS 的路线进行了详细的分析，在这个过程中，我们需要捋清楚 JS、C++ 和 Libuv 三层之间，数据结构的关联关系，否则就会陷入迷雾中。

了解了这些内容后，后面学习 TCP、UDP 等模块都会容易很多，同时也为我们自己写 JS 运行时打下了坚实的基础。