上节课介绍了 Node.js 的一些概念和组成，也讲到了 Node.js 的应用和执行架构，接下来我们要开始深入讲解 Node.js 的底层原理了。但想深入了解 Node.js，就必须先了解 Libuv，而想要了解 Libuv，则必须先了解它的核心数据结构。

  


因此，本节课我们会先详细介绍 Libuv 中的各种数据结构，包括事件循环、handle、request 族核心结构体，这也是后面章节的基础。此外，我们还会介绍 Libuv 中一些通用的逻辑，这是 Libuv 中很多地方都会用到的函数。

  


但我们并不会详细介绍这些字段的具体用处和原理，因为无法在一节课内全部讲完，所以我们先有个大概印象，后面学习的时候把它作为查阅的资料，再结合具体的场景分析，慢慢就能掌握了。
...
uv_loop_s 可以说是 Libuv 中最核心数据结构，也是理解事件循环的重要前提。每一个事件循环对应一个 uv_loop_s 结构体。它记录了整个事件循环中的核心数据。下面具体介绍一下每一个字段的意义。
...
uv_loop_s 是最复杂的数据结构，里面包括了非常多的字段，大家大概有个印象就好，后面需要的时候会详细介绍。
...
其他相关的结构体也类似这种方式实现了代码复用。我们看一下 uv_handle_t 的定义
...
handle 是 Libuv 中核心结构体，TCP、UDP 等数据结构都是 handle 类结构体。Libuv 中也提供了 init、start、stop、close 等操作 handle 结构体等函数。
...
uv_stream_s 用于表示一个流，比如 TCP 流，类似 JS 的 stream 机制，不过 Libuv 流可以控制数据的读写、连接、关闭等，后面的课程会具体分析。
...
uv_async_s 用于线程间的异步通信，父子线程通过 uv_async_s 就可以实现通信，具体细节由 Libuv 处理，后面课程会详细分析。
...
uv_tty_s 是和终端相关的结构体，因为本小册没有涉及这部分，所以这里只是简单介绍一下。
...
uv_fs_poll_s 用于监听文件变化，但是和 uv_fs_event_s 不一样的是，uv_fs_poll_s 是使用定时轮询的机制实现的，所以效率上比较低，但是兼容性更好，后面会具体分析。
...
Libuv 中，除了基础的数据结构外，还有一些针对这些数据结构的通用操作函数，这些操作函数逻辑并不复杂，但是在源码中到处存在，所以单独介绍一下，后面就不会具体分析。
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/fe70d76fe7184155b88e798efcbe0595~tplv-k3u1fbpfcp-zoom-1.image)
...
本节课主要介绍了 Libuv 中核心的数据结构和一些通用的函数，这些都是 Libuv 中的基础，是我们看懂 Libuv 的前提。所谓程序 = 数据结构 + 算法，了解了数据结构，再通过了解这些数据结构是如何被使用的，才能真正理解软件的实现。但是我们不需要对这些进行记忆，只需要大概了解一下，用到的时候再查阅就行。希望通过本章的学习，让大家后面的学习变得更轻松。