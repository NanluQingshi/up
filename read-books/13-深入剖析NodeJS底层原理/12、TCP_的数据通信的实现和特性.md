上一节课我们介绍了 TCP 协议的基础、使用以及 Node.js 中 TCP 服务器和客户端的实现。客户端和服务器建立了连接后只是通信的基础，那么有了连接后，是如何处理通信的呢，又是如何管理连接的呢？TCP 协议又有哪些特性是我们需要了解的呢？这正是这节课要讲的内容。
...
1.  有数据则把数据追加到流中，并触发 data 事件通知用户。
1.  没有有效数据可读，忽略。
1.  读出错，销毁流，即关闭连接。
1.  读结束。
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/39bdbc305ce74af6815ae87bd1393803~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e1ae23740dca4d37bb0822b1e01f4fac~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/82b20a613bf94cffb07eafc25312b832~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b8521837c422484ba25cc20ff103f1a6~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/7347d803512946568f79a6e960869c68~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/6e3482fa159d4eb990b6b144d819d188~tplv-k3u1fbpfcp-zoom-1.image)
...
1.  TIME_WAIT 状态的端口可以快速复用，否则需要等待 2 * MSL（Maximum Segment Lifetime） 时间。
1.  可以同时绑定全部 IP 和具体 IP。
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/8683b52803074fcca0b3db466a5cd2b2~tplv-k3u1fbpfcp-zoom-1.image)
...
1.  多久没有数据包通信，则开始发送探测包（有数据通信时则重置定时器）。
1.  每隔多久发送一次探测包。
1.  发送多少个探测包后，如果还没有回复，则断开连接。
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b0c3661a9780418e968fbc045b7e3cc6~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/8160e59824d64d9faf5736a2d7c9dbaa~tplv-k3u1fbpfcp-zoom-1.image)
...
![](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/1776c8f2a5f9475482440e9dcf60417f~tplv-k3u1fbpfcp-zoom-1.image)
...
1.  通过监听端口、监听 IP + 端口和监听不同的 IP 等场景介绍了 TCP 中监听不同地址的情况和原理，以后我们碰到相关问题时也可以迎刃而解，比如我们连接某个端口时报了 ECONNREFUSED 错误，但是通过 lsof 又发现监听了这个端口，则可以进一步判断是否监听时指定了 IP。
1.  通过 SO_REUSEADDR 和 SO_REUSEPORT 可以不同程度地实现多个服务器监听同一个端口，尤其是 SO_REUSEPORT 可以直接监听同一个 IP 和端口，使得我们可以在多个进程中共同处理连接，提高效率。
1.  关于 TCP Keepalive 机制，我们需要知道它的作用和使用场景。比如我们实现连接池时，就可以通过 Keepalive 机制探测空闲 socket 的对端是否还存活。
1.  最后是半开关的使用场景，如果我们想在 TCP 协议之上通过 JSON 格式的数据进行通信，客户端就可以通过关闭写端来通知服务器请求报文发送完毕，服务器再通过 JSON.parse 解析收到的数据，然后返回结果。否则，如果我们在一个连接中发送多个请求，服务器需要实现一个解析器来解析一个个请求。